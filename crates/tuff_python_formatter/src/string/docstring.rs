...
pub(crate) fn format(normalized: &NormalizedString, f: &mut PyFormatter) -> FormatResult<()> {
        ...
        .is_disabled()
    {
        ...
        aligned_text = align_google_args_sections(normalized_text);
        &aligned_text
    };
...
fn align_google_args_sections(docstring: &str) -> Cow<'_, str> {
    ...
    let mut markdown_fence: Option<MarkdownFenceMarker> = None;
    let mut rst_literal_block_indent: Option<usize> = None;

    while line_index < lines.len() {
        if let Some(block_indent) = rst_literal_block_indent {
            if lines[line_index].trim().is_empty()
                || leading_whitespace_count(&lines[line_index]) > block_indent
            {
                line_index += 1;
                continue;
            }

            rst_literal_block_indent = None;
        }

        if let Some(fence) = markdown_fence {
            if markdown_fence_end(&lines[line_index], fence) {
                markdown_fence = None;
            }
            line_index += 1;
            continue;
        ...
        if let Some(fence) = markdown_fence_start(&lines[line_index]) {
            markdown_fence = Some(fence);
            line_index += 1;
            continue;
        }

        if is_rst_literal_block_intro(&lines[line_index]) {
            rst_literal_block_indent = Some(leading_whitespace_count(&lines[line_index]));
            line_index += 1;
            continue;
        }

        if !is_google_args_section_header(&lines[line_index]) {
            line_index += 1;
            continue;
        }

        let section_indent = leading_whitespace_count(&lines[line_index]);
        line_index += 1;

        while line_index < lines.len() {
            if !lines[line_index].trim().is_empty()
                && leading_whitespace_count(&lines[line_index]) <= section_indent
            {
                break;
            }

            let Some(first_candidate) = google_arg_candidate(&lines[line_index], section_indent)
            else {
                if lines[line_index].trim().is_empty() {
                    break;
                }
            ...
            let entry_indent = first_candidate.indent;
            line_index += 1;
            ...
            while line_index < lines.len() {
                ...
                if line.trim().is_empty()
                    || leading_whitespace_count(line) <= section_indent && !line.trim().is_empty()
                {
                    break;
                }

                if google_arg_candidate(line, section_indent)
                    .is_some_and(|candidate| candidate.indent == entry_indent)
                    || leading_whitespace_count(line) > entry_indent
                {
                    line_index += 1;
                    continue;
                }
            ...
            changed |= align_google_arg_group(&mut lines[group_start..line_index], entry_indent);
        }
    }
...
fn is_rst_literal_block_intro(line: &str) -> bool {
...
fn markdown_fence_start(line: &str) -> Option<MarkdownFenceMarker> {
    ...
    (length >= 3).then_some(MarkdownFenceMarker { character, length })
}

fn markdown_fence_end(line: &str, fence: MarkdownFenceMarker) -> bool {
    let line = line.trim_start();
    let length = line
        .chars()
        .take_while(|candidate| *candidate == fence.character)
        .count();

    length >= fence.length
}

fn is_google_args_section_header(line: &str) -> bool {
    matches!(
        line.trim(),
        "Args:"
            | "Arguments:"
...
fn align_google_arg_group(lines: &mut [String], entry_indent: usize) -> bool {
    if lines.len() < 2 {
        return false;
    }

    let max_prefix_width = lines
        .iter()
        .filter_map(|line| google_arg_candidate(line, 0))
        .filter(|candidate| candidate.indent == entry_indent)
        .map(|candidate| candidate.prefix_width)
        .max()
        .unwrap_or_default();
    let mut changed = false;

    for line in lines {
        let Some(candidate) = google_arg_candidate(line, 0) else {
            continue;
        };
        if candidate.indent != entry_indent {
            continue;
        }

        let description = line[candidate.colon_index + 1..].trim_start();
        let spaces = " ".repeat(max_prefix_width.saturating_sub(candidate.prefix_width) + 1);
        let aligned = std::format!("{}:{spaces}{description}", &line[..candidate.colon_index]);
...
fn google_arg_candidate(line: &str, section_indent: usize) -> Option<GoogleArgCandidate> {
    ...
    let colon_index = google_arg_separator_colon(line, indent)?;
    let name = line[indent..colon_index].trim();
    let description = line[colon_index + 1..].trim();
    ...
    Some(GoogleArgCandidate {
        colon_index,
        indent,
        prefix_width: display_width_for_docstring(&line[..colon_index]),
    })
}

fn google_arg_separator_colon(line: &str, indent: usize) -> Option<usize> {
    let mut bracket_depth = 0_u32;
    let mut quote = None;
    let mut escaped = false;

    for (index, character) in line[indent..].char_indices() {
...
fn display_width_for_docstring(text: &str) -> usize {
    text.chars().fold(0_usize, |width, character| {
        if character == '\t' {
            width + 8 - width.rem_euclid(8)
        } else {
