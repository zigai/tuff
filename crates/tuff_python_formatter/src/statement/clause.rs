use ruff_formatter::{Argument, Arguments, FormatError, FormatOptions, write};
...
use ruff_python_trivia::{SimpleToken, SimpleTokenKind, SimpleTokenizer, indentation_at_offset};
use ruff_source_file::LineRanges;
use ruff_text_size::{Ranged, TextRange, TextSize};
use unicode_width::UnicodeWidthChar;

use crate::comments::{SourceComment, leading_alternate_branch_comments, trailing_comments};
use crate::custom::hooks::{self, OneLineSuiteClause, OneLineSuiteDecision};
    ...
    fn fmt(&self, f: &mut Formatter<PyFormatContext<'ast>>) -> FormatResult<()> {
        match should_suppress_clause(self, f)? {
            ...
            SuppressClauseHeader::No => {
                if one_line_suite_clause(self.header).is_some_and(|clause| {
                    hooks::one_line_suite_layout(
                        f,
                        clause,
                        self.body,
                        !self.trailing_colon_comment.is_empty(),
                    ) == OneLineSuiteDecision::PreferOneLine
                }) && one_line_suite_source_fits(self.header, self.body, f)?
                {
                    let Some(statement) = self.body.first() else {
                        return write!(f, [self.clause_header(), self.clause_body()]);
                    };
                    let one_line = format_with(|f| {
                        write!(
                            f,
                            [
                                self.clause_header(),
                    ...
                    let block =
                        format_with(|f| write!(f, [self.clause_header(), self.clause_body()]));

                    best_fitting![one_line, block]
...
fn one_line_suite_source_fits(
    header: ClauseHeader<'_>,
    body: &Suite,
    f: &PyFormatter,
) -> FormatResult<bool> {
    let source = f.context().source();
    let header_range = header.range(source)?;
    let Some(statement) = body.first() else {
        return Ok(false);
    };

    let indentation = indentation_width_at_offset(header_range.start(), source);
    let header_width = display_width(source[header_range].trim());
    let body_width = display_width(source[statement.range()].trim());
    let line_width = usize::from(f.options().line_width().value());

    Ok(indentation + header_width + 1 + body_width <= line_width)
}
...
fn indentation_width_at_offset(offset: TextSize, source: &str) -> usize {
    indentation_at_offset(offset, source)
        .unwrap_or_default()
        .chars()
...
fn one_line_suite_clause(value: ClauseHeader<'_>) -> Option<OneLineSuiteClause> {
    match value {
        ClauseHeader::If(_) => Some(OneLineSuiteClause::If),
        ClauseHeader::ElifElse(ElifElseClause { test: Some(_), .. }) => {
            Some(OneLineSuiteClause::Elif)
        }
        ClauseHeader::ElifElse(ElifElseClause { test: None, .. }) | ClauseHeader::OrElse(_) => {
            Some(OneLineSuiteClause::Else)
        }
        ClauseHeader::For(_) => Some(OneLineSuiteClause::For),
        ClauseHeader::While(_) => Some(OneLineSuiteClause::While),
        ClauseHeader::With(_) => Some(OneLineSuiteClause::With),
        ClauseHeader::Try(_) => Some(OneLineSuiteClause::Try),
        ClauseHeader::ExceptHandler(_) => Some(OneLineSuiteClause::Except),
        ClauseHeader::TryFinally(_) => Some(OneLineSuiteClause::Finally),
        ClauseHeader::MatchCase(_) => Some(OneLineSuiteClause::MatchCase),
        ClauseHeader::Class(_) | ClauseHeader::Function(_) | ClauseHeader::Match(_) => None,
    }
}
